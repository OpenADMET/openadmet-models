...
class PerimeterSplitter(SplitterBase):
    ...
    def split(self, X, Y):
        ...
        Split the data
        """
        splitter = PerimeterSplit(n_jobs=-1, test_size=self.test_size, random_state=self.random_state)
        train_idx, test_idx = next(splitter.split(X=X))
        return X[train_idx], X[test_idx], Y[train_idx], Y[test_idx]
...
class MaxDissimilaritySplitter(SplitterBase):
    ...
    def split(self, X, Y):
        ...
        Split the data
        """
        splitter = MaxDissimilaritySplit(n_jobs=-1, test_size=self.test_size, random_state=self.random_state)
        train_idx, test_idx = next(splitter.split(X=X))
        return X[train_idx], X[test_idx], Y[train_idx], Y[test_idx]