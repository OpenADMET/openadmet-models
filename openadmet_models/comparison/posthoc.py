...
class PostHocComparison(ComparisonBase):
    ...
    def anova(self, df, model_tags, save_dir=None):
        figure, axes = plt.subplots(1, len(self.metrics), sharex=False, sharey=False, figsize=(28, 8))
        for i, metric in enumerate(self.metrics):
    ...
    def tukeys_to_df(tukeys, cl=.95):
        ...
        for i in range(len(hsd.statistic)-1):
            for j in range(i+1, len(hsd.statistic)):
                ...
                stats.append(s)
                errorbars.append([s-hsd.confidence_interval(confidence_level=cl).low[i,j],
    ...
    def mcs_plots(self, df, model_tags, direction_dict=None, sig_levels=None, save_dir=None):
        figsize = (20,10)
        ...
        for i, metric in enumerate(self.metrics):
            ...
            significance[(hsd.pvalue < sig_levels[1]) & (hsd.pvalue >= sig_levels[2])] = '**'
            significance[(hsd.pvalue < sig_levels[0]) & (hsd.pvalue >= sig_levels[1])] = '*'
    ...
    def mean_diff_plots(self, df, model_tags, cl=None, save_dir=None):
        figure, axes = plt.subplots(len(self.metrics), 1, figsize=(8, 2 * len(self.metrics)), sharex=False)
        ax_ind = 0
        if not cl:
            cl = .95
